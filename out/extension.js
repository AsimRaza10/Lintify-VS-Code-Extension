...
function showWelcomePage() {
    const panel = vscode.window.createWebviewPanel('lintifyWelcome', '🧠 Welcome to Lintify', vscode.ViewColumn.One, { enableScripts: true });
...
async function reviewCode(code, language) {
    const apiKey = vscode.workspace.getConfiguration('lintify').get('apiKey');
    if (!apiKey) {
...
async function fixCode(code, language, editor, selection, documentInfo) {
    const apiKey = vscode.workspace.getConfiguration('lintify').get('apiKey');
    if (!apiKey) {
        vscode.window.showErrorMessage('Please set your Google AI API key in settings.');
        return;
...
\`\`\``;
            ...
            // Create a diff view
            await vscode.commands.executeCommand('vscode.diff', editor.document.uri, tempDoc.uri, 'Lintify: Original ↔ Fixed Code');
            // Ask user to confirm applying the fix
            const apply = await vscode.window.showInformationMessage('Do you want to apply these changes to your code?', 'Accept', 'Reject');
            if (apply === 'Accept') {
...
async function addCode(language, editor, documentInfo) {
    const apiKey = vscode.workspace.getConfiguration('lintify').get('apiKey');
    if (!apiKey) {
        vscode.window.showErrorMessage('Please set your Google AI API key in settings.');
        return;
...
${context ? `Context: ${context}` : ''}`;
            ...
            const insert = await vscode.window.showInformationMessage('Do you want to insert this code at the cursor position?', 'Insert', 'Cancel');
            if (insert === 'Insert') {
                // Check if the editor is still valid and the document is open
                const currentEditor = vscode.window.activeTextEditor;
                if (!currentEditor || currentEditor.document.uri.toString() !== editor.document.uri.toString()) {
...
function createOrShowPanel(extensionUri) {
    ...
    panel = vscode.window.createWebviewPanel('lintifyPanel', 'Lintify', column || vscode.ViewColumn.One, {
        enableScripts: true,
